...
void Fatal_Error(const char* function, const char* file, int line, const std::string& message)
{
    ...
    sprintf_s(buffer, "%s:%s:%d: %s", file, function, line, message.c_str());
...
void Check_Compliance()
{
    ...
    if (siSysInfo.dwPageSize != 4096) FATAL_ERROR("Page size different than 4KB detected");
...
void Deactivate_Page(void* data, const size_t size)
{
    ...
    //if (ret < 0) {
    //    char buffer[256];
    //    char* error_message = (char*)strerror_r(errno, buffer, 256);
