...
public:
    ...
    template <class Field_type = Struct_type>
    using Array_mask = SPGrid_Mask<log2_struct, NextLogTwo<sizeof(Field_type)>::value, dim, log2_page>;
    template <class Field_type = Struct_type>
    using Array_type = SPGrid_Array<Field_type, Array_mask<Field_type>>;

    template <class T1, class Field_type>
    typename std::enable_if<std::is_same<T1, Struct_type>::value, Array_type<Field_type>>::type
    ...
    template <class T1, class Field_type>
    typename std::enable_if<std::is_same<T1, Struct_type>::value, Array_type<const Field_type>>::type
