...
inline std::enable_if_t<isSize<TA>(1, 1) && isSize<TR>(1, 1) && isSize<TS>(1, 1)>
polarDecomposition(const Eigen::MatrixBase<TA>& A,
    Eigen::MatrixBase<TR>& R,
    Eigen::MatrixBase<TS>& S_Sym)
{
    S_Sym = A;
    R(0, 0) = 1;
}
...
/**
   \brief 1x1 SVD (singular value decomposition) A=USV'
   \param[in] A Input matrix.
...
singularValueDecomposition(
    ...
    Vector<T, 1>& Sigma,
    Vector<T, 1>& V,
    const T tol = 64 * std::numeric_limits<T>::epsilon())
{
    U(0, 0) = 1;
    V(0, 0) = 1;
    Sigma = A;
...
/**
  ...
  */
template <class T>
inline T wilkinsonShift(const T a1, const T b1, const T a2)
{
    using std::copysign;
    using std::fabs;
    using std::sqrt;
...
inline ZIRAN_FORCE_INLINE void process(Matrix<T, 3, 3>& B, Matrix<T, 3, 3>& U, Vector<T, 3>& sigma, Matrix<T, 3, 3>& V)
{
    int other = (t == 1) ? 0 : 2;
...
/**
  ...
  */
template <class T>
inline ZIRAN_FORCE_INLINE void flipSign(int i, Matrix<T, 3, 3>& U, Vector<T, 3>& sigma)
{
    sigma(i) = -sigma(i);
    U.col(i) = -U.col(i);
}
...
/**
  ...
  */
template <int t, class T>
inline std::enable_if_t<t == 0> sort(Matrix<T, 3, 3>& U, Vector<T, 3>& sigma, Matrix<T, 3, 3>& V)
{
    using std::fabs;
...
/**
  ...
  */
template <int t, class T>
inline std::enable_if_t<t == 1> sort(Matrix<T, 3, 3>& U, Vector<T, 3>& sigma, Matrix<T, 3, 3>& V)
{
    using std::fabs;
...
/**
  ...
  */
template <class T>
inline ZIRAN_FORCE_INLINE int singularValueDecompositionHelper(const Matrix<T, 3, 3>& A,
    Matrix<T, 3, 3>& U,
    Vector<T, 3>& sigma,
    Matrix<T, 3, 3>& V,
    T tol)
...
inline ZIRAN_FORCE_INLINE int singularValueDecomposition(const Matrix<float, 3, 3>& A,
    Matrix<float, 3, 3>& U,
    Vector<float, 3>& sigma,
    Matrix<float, 3, 3>& V,
    float tol = 128 * std::numeric_limits<float>::epsilon())
{
    return singularValueDecompositionHelper(A, U, sigma, V, tol);
}
inline ZIRAN_FORCE_INLINE int singularValueDecomposition(const Matrix<double, 3, 3>& A,
    Matrix<double, 3, 3>& U,
    Vector<double, 3>& sigma,
    Matrix<double, 3, 3>& V,
    double tol = 1024 * std::numeric_limits<double>::epsilon())
{
    return singularValueDecompositionHelper(A, U, sigma, V, tol);
}
...
/**
    ...
    */
template <class T>
inline ZIRAN_FORCE_INLINE void polarDecomposition(const Matrix<T, 3, 3>& A,
    Matrix<T, 3, 3>& R,
    Matrix<T, 3, 3>& S_Sym)
{
    Matrix<T, 3, 3> U;
